...
- **spec-guard** — Enforces spec-driven discipline for projects with existing specs (PRDs, RFCs, design docs, ADRs, acceptance criteria, API contracts). Three rules: every change traces to a spec item, a compliance tracker is maintained as a real artifact, every non-trivial change gets an integration plan BEFORE code. Drift detection catches scope creep and spec gaps. Pairs with the broader `anthropic-skills:spec-driven-dev` process framework.
